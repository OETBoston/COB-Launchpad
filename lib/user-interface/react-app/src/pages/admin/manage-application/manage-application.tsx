...
export default function ManageApplication() {
  ...
  const { data, onChange, errors, validate } = useForm({
    initialValue: () => {
      return application
        ? {
            ...
            maxTokens: application.maxTokens ?? 8192,
    ...
    validate: (form) => {
      ...
      if (form.systemPrompt.length > 2048) {
        errors.systemPrompt = "System prompt must be less than 2048 characters";
      }
      if (form.systemPromptRag.length > 2048) {
        errors.systemPromptRag =
          "System prompt with workspace must be less than 2048 characters";
      }
      if (form.condenseSystemPrompt.length > 2048) {
