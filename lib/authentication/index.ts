...
export class Authentication extends Construct {
  ...
  constructor(scope: Construct, id: string, config: SystemConfig) {
    ...
    new cognito.CfnUserPoolGroup(this, "AdminGroup", {
      ...
      groupName: "admin",
      description: "Administrators group, full access to all features",
    });

    new cognito.CfnUserPoolGroup(this, "WorkspaceManagerGroup", {
      userPoolId: userPool.userPoolId,
      groupName: "workspace_manager",
      description: "Workspace managers group, access to creating and managing RAG workspaces",
    });

    new cognito.CfnUserPoolGroup(this, "ChatbotUserGroup", {
      userPoolId: userPool.userPoolId,
      groupName: "chatbot_user",
      description: "Chatbot users group, basic access to playground and session history",
    });
        ...
        const customProvider = new cognito.UserPoolIdentityProviderOidc(
          ...
          "OIDCProvider",
          {
            ...
            ...(config.cognitoFederation?.customOIDC?.OIDCAuthorizationEndpoint &&
               config.cognitoFederation?.customOIDC?.OIDCJWKSURI &&
               config.cognitoFederation?.customOIDC?.OIDCTokenEndpoint &&
               config.cognitoFederation?.customOIDC?.OIDCUserInfoEndpoint && {
                 endpoints: {
                   authorization: config.cognitoFederation.customOIDC.OIDCAuthorizationEndpoint,
                   jwksUri: config.cognitoFederation.customOIDC.OIDCJWKSURI,
                   token: config.cognitoFederation.customOIDC.OIDCTokenEndpoint,
                   userInfo: config.cognitoFederation.customOIDC.OIDCUserInfoEndpoint,
                 }
            }),
          }
        );
